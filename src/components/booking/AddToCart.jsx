...
const AddToCart = () => {
  ...
  const handleSubmitBookingRequest = async () => {
    ...
    const incompleteItems = selectedItemsData.filter(
      (item) => !hasCompleteBookingInfo(item)
    );
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setErrorCheckout("Failed to process payment.");
    ...
    currentItems.forEach((item) => {
      if (activeTab === "accepted" || hasCompleteBookingInfo(item)) {
        if (item.pricing?.perEvent) {
          ...
          subtotal += item.pricing.perHour * 8;
  ...
  const renderCartItem = (item, index) => {
    ...
    return (
      <div
        key={item.id || index}
        className="flex items-center justify-between p-4 bg-white rounded-xl border border-gray-100 mb-4"
      >
